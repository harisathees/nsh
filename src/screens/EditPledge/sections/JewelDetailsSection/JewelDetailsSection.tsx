...
import { JewelData } from "../../EditPledge";
...
export const JewelDetailsSection = ({
  ...
  const addNewJewel = async () => {
    ...
    // For demo mode or when loanId is 'test', just add to state
    if (loanId === 'test') {
      onJewelDataChange([...jewelData, newJewel]);
      return;
    }

    // For real data, insert into database
    try {
      ...
      onJewelDataChange([...jewelData, data]);
    } catch (err) {
      console.error('Error adding new jewel:', err);
      alert('Failed to add new jewel. Please try again.');
    }
  };

  const removeJewel = async (index: number) => {
    const jewelToRemove = jewelData[index];
    
    // For demo mode, just remove from state
    if (loanId === 'test') {
    ...
    try {
      const { error } = await supabase
        .from('jewels')
        .delete()
        .eq('id', jewelToRemove.id);

      if (error) {
        console.error('Error removing jewel:', error);
        throw error;
      }
      ...
      console.error('Error removing jewel:', err);
      alert('Failed to remove jewel. Please try again.');
    }
  };
  ...
  const handleImageUpload = async (index: number, event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const jewel = jewelData[index];
    const uploadKey = `jewel_${index}`;

    try {
      setUploading(prev => ({ ...prev, [uploadKey]: true }));
      
      const filePath = generateJewelImagePath(jewel.id, file.name);
  ...
  return (
    ...
    <div className="space-y-6">
      {jewelData.map((jewel, index) => (
        <Card key={jewel.id || index} className="w-full rounded-[30px] bg-white shadow-lg relative">
              ...
              {formFields.map((field) => (
                <div key={field.id} className="relative">
                  <Input
                    ...
                    className="h-[50px] px-4 py-3 border-[#eca94a] bg-white rounded-[30px] border border-solid text-gray-700 text-sm focus:ring-2 focus:ring-[#fff5c5] focus:border-[#eca94a]"
                  />
                </div>
              ))}
            </div>
      ...
      <div className="flex justify-center">
        <Button
          onClick={addNewJewel}
          className="flex items-center gap-2 h-12 px-6 py-3 bg-[#fff5c5] rounded-[31.5px] text-[#ea9f39] hover:bg-[#f0e8a8] border border-[#ea9f39]"
        >
