...
import { CustomerData } from "../../EditPledge";
...
export const CustomerDetailsSection = ({
  ...
  const handlePhotoUpload = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const filePath = generateCustomerImagePath(customerData.id, file.name);
  ...
  const handleAudioUpload = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const filePath = generateCustomerAudioPath(customerData.id, file.name);
  ...
  const formFields = [
    { id: "name", label: "Name", type: "input", value: customerData.name },
    { id: "mobile_no", label: "Mobile No", type: "input", value: customerData.mobile_no },
    ...
    { id: "id_proof", label: "Id Proof", type: "input", value: customerData.id_proof },
  ];
  ...
  return (
    ...
    <Card className="relative w-full rounded-[30px] overflow-hidden bg-white shadow-lg">
      <CardContent className="p-0">
        {/* Yellow header */}
        <div className="relative w-full bg-[#fff5c5] rounded-t-[30px] py-8 mb-6">
          <div className="absolute bottom-4 left-0 right-0 text-center font-semibold text-[#ea9f39] text-xl">
            Edit Pledge
          </div>
        </div>
        ...
        <div className="px-7 pb-6">
          ...
          <div className="relative mb-6">
            <div className="flex items-center gap-2 font-normal text-black text-lg">
              <UsersIcon className="w-6 h-6 text-[#ea9f39]" />
              Customer Details
            </div>
          </div>
              ...
              <div key={field.id} className="relative">
                {field.type === "input" ? (
                  <Input
                    ...
                    className="h-[50px] px-4 py-3 rounded-[30px] border border-solid border-[#ea9f39] focus:ring-2 focus:ring-[#fff5c5] focus:border-[#ea9f39]"
                  />
                ) : (
                  <Textarea
                    placeholder={field.label}
                    value={field.value || ""}
                    onChange={(e) => handleInputChange(field.id as keyof CustomerData, e.target.value)}
                    className="min-h-[100px] px-4 py-3 rounded-[30px] border border-solid border-[#ea9f39] focus:ring-2 focus:ring-[#fff5c5] focus:border-[#ea9f39] resize-none"
                  />
                )}
              </div>
            ))}
