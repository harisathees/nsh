...
// Define a type for your customer suggestions
type CustomerSuggestion = Omit<CustomerData, 'photo_url' | 'audio_url'> & {
  id: number; // Assuming your customer table has a unique id
...
export const CustomerDetailsSection = ({
  ...
  const [activeField, setActiveField] = useState<keyof CustomerData | null>(null);

  // --- NEW: Function to fetch customer suggestions from Supabase ---
  const fetchSuggestions = useCallback(async (field: keyof CustomerData, query: string) => {
    ...
    try {
      const { data, error } = await supabase
        .from('customers') // IMPORTANT: Replace 'customers' with your actual table name
        .select('*')
  ...
  // --- NEW: Debounce effect for fetching ---
  useEffect(() => {
    if (activeField && customerData[activeField]) {
      const handler = setTimeout(() => {
        const query = customerData[activeField];
        if (typeof query === 'string') {
          fetchSuggestions(activeField, query);
        }
      }, 300); // 300ms delay
  ...
  }, [customerData, activeField, fetchSuggestions]);
  ...
  const handleInputChange = (field: keyof CustomerData, value: string) => {
    ...
    // Set the current field as active to show suggestions for it
    setActiveField(field);
  };

  // --- NEW: Function to handle when a suggestion is clicked ---
  const handleSuggestionClick = (suggestion: CustomerSuggestion) => {
    onCustomerDataChange({ ...customerData, ...suggestion });
    setSuggestions([]); // Clear suggestions
    setActiveField(null); // Hide dropdown
  };


  // --- All your existing handle functions (handlePhotoUpload, etc.) remain unchanged ---
  const handlePhotoUpload = async (event: React.ChangeEvent<HTMLInputElement>) => {
    const file = event.target.files?.[0];
    if (!file) return;
  ...
  const handleAudioUpload = async (event: React.ChangeEvent<HTMLInputElement>) => {
    const file = event.target.files?.[0];
    if (!file) return;
    if (!file.type.startsWith('audio/')) { alert('Please select a valid audio file'); return; }
    if (file.size > 10 * 1024 * 1024) { alert('Audio size should be less than 10MB'); return; }
    try {
      setUploading(prev => ({ ...prev, audio: true }));
      const tempCustomerId = `temp_${Date.now()}`;
      const filePath = generateCustomerAudioPath(tempCustomerId, file.name);
  ...
  // --- End of existing handle functions ---

  const formFields = [
    { id: "name", label: "Name", type: "input", required: true },
    { id: "mobile_no", label: "Mobile No", type: "input", required: true },
    { id: "whatsapp_no", label: "Whatsapp No", type: "input" },
  ...
  return (
    <>
      ...
      <CameraCapture isOpen={showCamera} onCapture={handleCameraCapture} onClose={() => setShowCamera(false)} />
      <AudioRecorder isOpen={showAudioRecorder} onCapture={handleAudioCapture} onClose={() => setShowAudioRecorder(false)} />

      <Card className="relative w-full rounded-[30px] overflow-hidden bg-white shadow-lg">
        <CardContent className="p-0">
          <div className="relative w-full bg-[#C7EEFF] rounded-t-[30px] py-8 mb-6">
            <div className="absolute bottom-4 left-0 right-0 text-center font-semibold text-[#014F70] text-xl">Create Pledge</div>
          </div>
          ...
          <div className="px-7 pb-6">
            ...
            <div className="space-y-4">
              {formFields.map((field) => (
                <div key={field.id} className="relative"> {/* IMPORTANT: Added relative positioning */}
                  <label className="block mb-5 text-sm font-medium text-black-700 px-2">
                    {field.label}{field.required ? " *" : ""}
                  </label>
                  {field.type === "input" ? (
                    <Input
                      placeholder={field.label}
                      value={customerData[field.id as keyof CustomerData] as string || ""}
                      onChange={(e) => handleInputChange(field.id as keyof CustomerData, e.target.value)}
                      onFocus={() => setActiveField(field.id as keyof CustomerData)} // Show suggestions on focus
                      onBlur={() => setTimeout(() => setActiveField(null), 200)} // Hide with delay
                      className={`h-[50px] px-4 py-3 rounded-[30px] border border-solid focus:ring-2 focus:ring-[#fff5c5] ${field.required ? 'border-[#269AD4]' : 'border-[#269AD4]'} focus:border-[#269AD4]`}
                      required={field.required}
                    ...
                    <Textarea
                      ...
                      value={customerData[field.id as keyof CustomerData] as string || ""}
                      onChange={(e) => handleInputChange(field.id as keyof CustomerData, e.target.value)}
                      onFocus={() => setActiveField(field.id as keyof CustomerData)}
                  ...
                  {activeField === field.id && (suggestions.length > 0 || isSuggestionLoading) && (
                    <div className="absolute top-full left-0 right-0 bg-white border rounded-lg shadow-lg z-20 mt-1 max-h-60 overflow-y-auto">
                      {isSuggestionLoading ? (
                        ...
                        suggestions.map((suggestion) => (
                          <button
                            ...
                            className="w-full text-left px-4 py-3 hover:bg-gray-100 text-sm border-b last:border-b-0"
                          >
                            <div className="font-medium text-black">{suggestion.name}</div>
                            <div className="text-gray-600 text-xs">{suggestion.mobile_no}</div>
                          </button>
                        ))
                      ) : null}
                    </div>
              ...
              {/* Your existing ID Proof and Media capture sections remain unchanged */}
              <div className="flex flex-col gap-1 ">
                <label className="font-medium px-4">ID Proof</label>
                <div className="flex gap-2 px-0 ">
                  <select
